program BRIO
  ...
  !
  ! Created/last modified: jun 26, 2013/oct 24, 2013
  !
  ...
  if(myrank.eq.0) then
     ...
     if(inline)then
        ...
        print*, "  - with non-contiguous writing"
     endif
#endif
#if POTOK == 1
     if(potok) print*, "  - with ", ntoken, " token"
#endif
#if ADIOS == 1
     if(adios) then
        if(xml)then
  ...
  ! Write & read files
#if POSIX == 1 || POTOK == 1
  if(posix .or. potok) call create_dir(myrank)
#endif
#if POSIX == 1
  if(posix) then
     tbegin_px = MPI_Wtime()
  ...
  if(potok) then
     ...
     if(myrank >= first .and. myrank <= last) then
        if(mod(myrank, nparallel) .ne. 0) then
           call MPI_Recv(token, 1, MPI_INT, myrank - 1, tag, MPI_COMM_WORLD &
     ...
     if(myrank >= first .and. myrank <= last) then
        ...
        call write_posix(data, xpos, ypos, zpos, myrank)
        if(mod((myrank + 1), nparallel) .ne. 0 .and. myrank < last) then
           call MPI_Send(token, 1, MPI_INT, myrank + 1, tag, MPI_COMM_WORLD &
                , ierr)
        endif
     endif
     call MPI_Barrier(MPI_COMM_WORLD, ierr)
  ...
  if(mpiio) then
     ...
     tbegin_mp = MPI_Wtime()
     call read_mpiio(data, xpos, ypos, zpos, myrank)
     call MPI_Barrier(MPI_COMM_WORLD, ierr)
     tend_mp = MPI_Wtime()
     if(myrank==0) write(*,*) 'MPI-IO > reading speed: ' &
          , totalsize/(tend_mp - tbegin_mp)/1024**2, 'MiB/s'  
  endif
#endif
  ...
  contains
    ...
    subroutine read_params
      ...
      namelist /output_params/ posix, potok, pncdf, phdf5, adios, mpiio &
           ,inline, ntoken
